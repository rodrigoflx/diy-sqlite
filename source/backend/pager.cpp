...
std::uint32_t Pager::get_num_pages() {
    return num_pages;
}
...
std::shared_ptr<Page> Pager::get_page(int page_number) {
    ...
    if (!entry.is_valid) {
        ...
        file_stream.read(reinterpret_cast<char*>(cache.get() + (page_number % CACHE_PAGES) * PAGE_SIZE), PAGE_SIZE);
...
void Pager::flush(int page_number) {
    ...
    file_stream.seekp(offset);
    file_stream.write(reinterpret_cast<const char*>(cache.get() + cache_offset), PAGE_SIZE);
    file_stream.flush();
}
...
auto create_pager(const std::filesystem::path& filename) -> std::unique_ptr<Pager> {
...
void Pager::evict_page(size_t cache_index) {
    ...
    if (entry.is_valid && entry.is_dirty) {
        ...
        file_stream.seekp(offset);
        file_stream.write(reinterpret_cast<const char*>(cache.get() + cache_index * PAGE_SIZE), PAGE_SIZE);
    }
