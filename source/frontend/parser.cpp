...
{
  if (auto r = consume(token_type::keyword, "SELECT"); !r)
  ...
  if (peek().value == "*") {
    ...
    while (true) {
      if (const auto col = consume(token_type::identifier, ""); col.has_value())
  ...
  if (const auto table = consume(token_type::identifier, ""); table.has_value())
...
tl::expected<join_clause, parse_error> parser::parse_join_clause()
{
  ...
  condition cond;
  if (const auto left = consume(token_type::identifier, ""); left.has_value())
    cond.column = left.value();
  else
    return tl::make_unexpected(left.error());
  if (auto eq = consume(token_type::operator_, "="); !eq)
    return tl::make_unexpected(eq.error());
  if (const auto right = consume(token_type::identifier, ""); right.has_value())
...
tl::expected<insert_statement, parse_error> parser::parse_insert()
{
  ...
  while (true) {
    if (auto col = consume(token_type::identifier, ""); col.has_value())
      stmt.columns.push_back(col.value());
    else
      return tl::make_unexpected(col.error());
    if (peek().type == token_type::punctuation && peek().value == ",") {
  ...
  while (true) {
    if (const auto val = consume(token_type::literal, ""); val.has_value())
...
tl::expected<update_statement, parse_error> parser::parse_update()
{
  ...
  while (true) {
    std::string col;
    if (const auto col_tok = consume(token_type::identifier, ""); col_tok.has_value())
      col = col_tok.value();
    else
    ...
    if (auto eq = consume(token_type::operator_, "="); !eq)
      return tl::make_unexpected(eq.error());
    if (const auto val = consume(token_type::literal, ""); val.has_value())
      stmt.assignments.emplace_back(col, val.value());
    else 
      return tl::make_unexpected(val.error());
    if (peek().type == token_type::punctuation && peek().value == ",") {
      if (auto comma = consume(token_type::punctuation, ","); !comma)
...
tl::expected<condition, parse_error> parser::parse_condition()
{
  ...
  if (const auto col = consume(token_type::identifier, ""); col.has_value())
    ...
    return tl::make_unexpected(col.error());
  if (auto op = consume(token_type::operator_, ""); op.has_value())
  ...
  if (auto val = consume(token_type::literal, ""); !val)
    return tl::make_unexpected(val.error());
